...
def visualize_results(x_normal, x_lesion, x_recon, mask_lesion, anomaly_map, fusion_weights, save_path):
    ...
    Args:
        x_normal: [B, 1, H, W] ground truth
        x_lesion: [B, 1, H, W] input with lesion
        x_recon: [B, 1, H, W] reconstruction
    ...
    # Compute residual
    residual = np.abs(x_lesion - x_recon)
...
if __name__ == "__main__":
    ...
    # Dummy data
    B = 2
    x_normal = torch.rand(B, 1, 256, 256)
    x_lesion = torch.rand(B, 1, 256, 256)
    x_recon = torch.rand(B, 1, 256, 256)
